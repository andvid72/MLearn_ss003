...
#Seleccionar columnas de tipo entero o flotante, devuelve una parte de la tabla
var_numericas_df = mi_data.select_dtypes([int, float])
var_numericas_df.columns

#Devolver las filas que les faltan datos
var_numericas_df[var_numericas_df.isnull().any(axis=1)]

#Define la estrategia con la que se rellenan valores faltantes
imputador = SimpleImputer(missing_values=np.nan, copy=False, strategy="mean")

#Obtenemos las filas corregidas
...
#Verificación de que ya no faltan datos
...
#Funciones para obtener las medias y desviaciones estándar de un dataframe
...
#El escalador hace la transformación de (dato-media)/std
escalador = preprocessing.StandardScaler()
#Aplicado del escalador, devuelve un array
var_numericas_imputadas_escalado_standard = escalador.fit_transform(var_numericas_imputadas_df)

#Propiedades del escalador
escalador.mean_
np.sqrt(escalador.var_)

#Comprobar que la media y la std son 1 y cero después de la transformación
...
#TABLA SIN HUECOS Y ESTANDARIZADA EN CADA COLUMNA
...
#Constructor de escalador robusto a través de la biblioteca preprocessing
